...
@section('panel-header')
    <div>
        <ul class="breadcrumb">
            ...
            <li><i data-feather="chevron-right"></i></li>
            <li><a href="{{ route('admin.view.coupon.create') }}">Add Coupon</a></li>
        </ul>
        <h1 class="panel-title">Add Coupon</h1>
    </div>
@endsection
    ...
    <form action="{{ route('admin.handle.coupon.create') }}" method="POST" enctype="multipart/form-data">
        ...
        <figure class="panel-card">
            ...
            <div class="panel-card-body">
                <div class="grid 2xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1 gap-5">
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="discount_type" class="input-box-md @error('discount_type') input-invalid @enderror" required>
                            <option value="">Select Type</option>
                            @foreach ($discount_type::cases() as $type)
                                <option @selected(old('discount_type') == $type->value) value="{{ $type->value }}">
                                    {{ $type->label() }}</option>
                            @endforeach
                        </select>
                        @error('discount_type')
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="discount_value" value="{{ old('discount_value') }}"
                            class="input-box-md @error('discount_value') input-invalid @enderror" placeholder="Enter Discount Value"
                    ...
                    {{-- Minimum Purchase --}}
                    <div class="input-group">
                        <label for="minimum_purchase" class="input-label">Minimum Purchase <em>*</em> <span>(In {{config('app.currency.code')}})</span></label>
                        <input type="number" name="minimum_purchase" value="{{ old('minimum_purchase') }}"
                            class="input-box-md @error('minimum_purchase') input-invalid @enderror" placeholder="Enter Minimum Purchase"
                    ...
                    {{-- Maximum Discount --}}
                    <div class="input-group">
                        <label for="maximum_discount" class="input-label">Maximum Discount <em>*</em> <span>(In {{config('app.currency.code')}})</span></label>
                        <input type="number" name="maximum_discount" value="{{ old('maximum_discount') }}"
                            class="input-box-md @error('maximum_discount') input-invalid @enderror" placeholder="Enter Maximum Discount"
            ...
            <div class="panel-card-footer">
                <button type="submit" class="btn-primary-md md:w-fit sm:w-full">Add Coupon</button>
            </div>
        </figure>
    </form>
@endsection
