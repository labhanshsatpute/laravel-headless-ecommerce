...
@section('panel-header')
    <div>
        <ul class="breadcrumb">
            ...
            <li><i data-feather="chevron-right"></i></li>
            <li><a href="{{ route('admin.view.product.update', ['id' => $product->id]) }}">Edit Product</a></li>
        </ul>
            ...
            <div class="panel-card-body">
                <div class="grid 2xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1 gap-5">
                    ...
                    <div class="input-group">
                        ...
                        <select name="parent_category_id"
                            ...
                            @foreach ($parent_categories as $category)
                                <option @selected(old('parent_category_id', $product->parent_category_id) == $category->id) value="{{ $category->id }}">
                    ...
                    <div class="input-group">
                        ...
                        <select name="child_category_id"
                            ...
                            @foreach ($child_categories as $category)
                                <option @selected(old('child_category_id', $product->child_category_id) == $category->id) value="{{ $category->id }}">
                                    {{ $category->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="input-group 2xl:col-span-5 lg:col-span-4 md:col-span-2 sm:col-span-1">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="description" id="description"
                                value="{{ old('description', $product->description) }}" hidden>
                        </div>
                        <script>
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="any" name="price_original"
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="any" name="price_discounted"
                            value="{{ old('price_discounted', $product->price_discounted) }}"
                    ...
                    <div class="flex flex-col md:col-span-4 sm:col-span-1">
                        ...
                        <textarea name="meta_description" rows="3"
                            ...
                            minlength="1" maxlength="1000">{{ old('meta_description', $product->meta_description) }}</textarea>
                    ...
                    <div class="input-group">
                        ...
                        <input type="color" name="color" value="{{ old('color', $product->color) }}"
                            class="h-12 w-12">
                        @error('color')
                            <span class="input-error">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="input-group 2xl:col-span-5 lg:col-span-4 md:col-span-2 sm:col-span-1">
                        ...
                        <div class="flex space-x-3 my-2">
                            ...
                            <img src="{{ is_null($product->thumbnail_image) ? asset('admin/images/default-thumbnail.png') : asset('storage/' . $product->thumbnail_image) }}"
                            ...
                            <div class="grid md:grid-cols-4 sm:grid-cols-2 gap-5">
                                @foreach ($product->media as $media)
                                    <div class="space-y-2 p-2 border rounded-md">
                                        <figure
                                            class="border rounded-md h-[200px] w-full flex items-center justify-center bg-slate-200 overflow-clip">
                                            @switch($media->type)
                                                @case('image/jpeg')
                                                @case('image/png')
                                                ...
                                                @case('video/mp4')
                                                    <video src="{{ asset('storage/' . $media->path) }}" controls
                                                        class="h-full w-auto"></video>
                                                @break

                                                @default
                                                    <a target="_blank" href="{{ asset('storage/' . $media->path) }}"
                                                        class="btn-secondary-md">Preview Media</a>
                                            @endswitch
                                        </figure>
                                        <button type="button" onclick="handleMediaDelete({{ $media->id }})"
                                            class="btn-danger-sm w-full">Delete</button>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="space-x-3 flex">
                            @foreach ($availablity::cases() as $item)
                                <div class="input-radio">
                                    <input type="radio" value="{{ $item->value }}" @checked(old('availability', $product->availability) == $item->value)
...
@section('panel-script')
    <script>
        ...
        const handleThumbnailPreview = (event) => {
            if (event.target.files.length == 0) {
                document.getElementById('thumbnail_image').src =
                    "{{ is_null($product->thumbnail_image) ? asset('admin/images/default-thumbnail.png') : asset('storage/' . $product->thumbnail_image) }}";
            } else {
        ...
        const handleSizeDelete = (id) => {
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        window.location =
                            `{{ url('admin/product/size/delete') }}/${id}`;
                    }
                });
        }
        ...
        const handleCreateSizeInput = (value, price_original, price_discounted, id = null) => {
    ...
    @if (!is_null($product->sizes))
        <script defer>
            @foreach (json_decode($product->sizes) as $size)
                handleCreateSizeInput('{{ $size->size }}', {{ $size->price_original }}, {{ $size->price_discounted }},
                    {{ $size->id }});
            @endforeach
        </script>
    @endif
